...
def test_document_processing():
    ...
    try:
        ...
        course_files = [f for f in os.listdir(full_docs_path) if f.lower().endswith(('.txt', '.pdf', '.docx'))]
        if not course_files:
...
def test_course_loading_to_vector_store():
    ...
    try:
        ...
        courses_added, chunks_added = rag_system.add_course_folder(full_docs_path, clear_existing=False)
