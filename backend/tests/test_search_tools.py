...
class TestCourseSearchTool(unittest.TestCase):
    ...
    def test_execute_with_lesson_filter(self):
        ...
        result = self.search_tool.execute(query="pandas dataframes", lesson_number=5)

        self.mock_vector_store.search.assert_called_once_with(
            query="pandas dataframes", course_name=None, lesson_number=5
        )
    ...
    def test_execute_empty_results_with_filters(self):
        ...
        result = self.search_tool.execute(query="topic", course_name="Machine Learning")

        self.assertIn("No relevant content found", result)
        self.assertIn("in course 'Machine Learning'", result)

        # Test with lesson filter
        result = self.search_tool.execute(query="topic", lesson_number=3)
